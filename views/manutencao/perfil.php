<div id="section-container">
    <div class="row">
        <div class="col-sm-12 col-md-12 col-lg-12" id="pagina-header">
            <h2 class="text-uppercase">Ordem de Manutenção: <?php echo !empty($manutencao['id']) ? "Nº " . str_pad($manutencao['id'], 4, '0', STR_PAD_LEFT) : '' ?></h2>
            <ol class="breadcrumb">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td width="120px" class=" text-center">
                                        <a class="btn btn-primary btn-xs" href="<?php echo BASE_URL . '/editar/desvio/' . $index['id']; ?>" title="Editar"><i class="fa fa-pencil-alt"></i></a>
        ...
        <div class="col-md-12 clear">
            <section class="panel panel-black">
                <header class="panel-heading">
                    <h4 class="panel-title"><i class="fa fa-calendar-alt  pull-left"></i> Encerrar</h4>
                </header>
                <article class="panel-body">
                    <span class="pull-right"><a href="<?php echo BASE_URL . '/cadastrar/encerrar/' . $manutencao['id'] ?>" class="btn btn-sm btn-warning" title="Adicionar"><i class="fa fa-plus-circle"></i> Adicionar</a></span>
                    <?php
                    if (isset($manutencao['encerrar']) && !empty($manutencao['encerrar'])) :
                    ?>
                        <br>
                        <br>
                        <table class="table table-striped table-bordered table-hover table-condensed">
                            ...
                            <?php
                            $qtd = 1;
                            foreach ($manutencao['encerrar'] as $index) :
                            ?>
                                <tr>
                                    <td width="40px"><?php echo $qtd; ?></td>
                                    <td>
                                    ...
                                    <td><?= !empty($index['duracao']) ? $index['duracao'] : '' ?></td>
                                    <td>
                                        <?php if (!empty($index['created_encerrar'])) : ?>
                                            Cadastro: <?= (!empty($index['created_encerrar'])) ? $this->viewDateComplet($index['created_encerrar']) : '' ?>
                                        <?php endif; ?>
                                        <?php if (!empty($index['updated_encerrar'])) : ?>
                                            <br />
                                            Alteração: <?= (!empty($index['updated_encerrar'])) ? $this->viewDateComplet($index['updated_encerrar']) : '' ?>
                                        <?php endif; ?>
                                    </td>
                                    <td><?= !empty($index['descricao']) ? $index['descricao'] : '' ?></td>
                                    <td width="120px" class=" text-center">
                                        <a class="btn btn-primary btn-xs" href="<?php echo BASE_URL . '/editar/encerrar/' . $index['id']; ?>" title="Editar"><i class="fa fa-pencil-alt"></i></a>
                                        <button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#modal_encerrar_<?php echo $index['id']; ?>" title="Excluir"><i class="fa fa-trash"></i></button>
                                    </td>
                                </tr>
                            <?php
                                ++$qtd;
                ...
                <section class="modal-content">
                    ...
                    <article class="modal-body">
                        <ul class="list-unstyled">
                            ...
                            <li><b>Ordem: </b> <?php echo !empty($index['ordem']) ? $index['ordem'] : '' ?>;</li>
                            <li><b>Típo: </b> <?php echo !empty($index['tipo']) ? $index['tipo'] : '' ?>;</li>
                            <li><b>Duração: </b> <?php echo !empty($index['duracao']) ? $index['duracao'] : '' ?>;</li>
                            <li><b>Descrição: </b> <?php echo !empty($index['descricao']) ? $index['descricao'] : '' ?>;</li>
                        </ul>
                    ...
                    <footer class="modal-footer">
                        <a class="btn btn-danger pull-left" href='<?php echo BASE_URL . "/excluir/desvio/{$index['manutencao_id']}/{$index['id']}" ?>'> <i class="fa fa-trash"></i> Excluir</a>
...
?>
        <!--MODAL - ESTRUTURA BÁSICA-->
        <section class="modal fade" id="modal_encerrar_<?php echo $index['id'] ?>" tabindex="-1" role="dialog">
            <article class="modal-dialog modal-md" role="document">
                <section class="modal-content">
                    <header class="modal-header bg-primary">
                        <button class="close" type="button" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    ...
                    <article class="modal-body">
                        <ul class="list-unstyled">
                            <li><b>Código: </b> <?php echo !empty($index['id']) ? $index['id'] : '' ?>;</li>
                            <li><b>Ocorrência de desvio: </b> <?php echo !empty($index['ordem']) ? $index['ordem'] : '' ?>;</li>
                            <li><b>Duração: </b> <?php echo !empty($index['duracao']) ? $index['duracao'] : '' ?>;</li>
                            <li><b>Descrição: </b> <?php echo !empty($index['descricao']) ? $index['descricao'] : '' ?>;</li>
                        </ul>
                        <p class="text-justify text-danger"><span class="font-bold">OBS : </span> Se você remove este registro, o mesmo deixará de existir no sistema.</p>
                    ...
                    <footer class="modal-footer">
                        <a class="btn btn-danger pull-left" href='<?php echo BASE_URL . "/excluir/encerrar/{$index['manutencao_id']}/{$index['id']}" ?>'> <i class="fa fa-trash"></i> Excluir</a>
                        <button class="btn btn-default" type="button" data-dismiss="modal"><i class="fa fa-times"></i> Fechar</button>
                    </footer>
                </section>
            </article>
